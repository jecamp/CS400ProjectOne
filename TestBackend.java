...
public class TestBackend {
    ...
    /**
     * This test instantiates the back end with three movies and tests whether the
     * initial list correctly adds the genre to the list. It passes when 0 is
    ...
    public boolean testAddGenre() {
        try {
            ...
            if (backendToTest.getAllGenres().size() == 6
                    ...
                    && backendToTest.getAllGenres().contains("Musical")
                    && backendToTest.getAllGenres().contains("Romance")
                            && backendToTest.getAllGenres().contains("Western")) {
                // test passed
                return true;
            } else {
                // test failed
    ...
    /**
     * This test instantiates the back end with three movies and tests whether the
     * initial list correctly removes the genre in the list. It passes when 0 is
     * returned and fails in all other cases, including when an exception is thrown.
     * @return true if the test passed, false if its failed
     */
    public boolean testRemoveGenre() {
        try {
            ...
            if (backendToTest.getAllGenres().size() == 4
                    && backendToTest.getAllGenres().contains("Action")
                    && backendToTest.getAllGenres().contains("Comedy")
                    && backendToTest.getAllGenres().contains("Musical")
                    && backendToTest.getAllGenres().contains("Romance")) {
